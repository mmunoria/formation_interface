...
Real single-drone testing (no fleet simulation): each real drone bridges its
own OptiTrack pose onto DronePose and publishes its DroneActive heartbeat by
running the standalone scripts in scripts/ directly (not part of this launch
file, since each instance runs on its own drone's companion computer, not the
host PC):

    python3 scripts/drone_active_publisher.py --ros-args -p drone_id:=1
    python3 scripts/optitrack_pose_bridge.py --ros-args -p drone_id:=1
